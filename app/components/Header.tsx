...
export default function Header() {
  ...
  const getLinkClass = (href: string) => {
    const baseClass = "text-white uppercase tracking-widest hover:text-primary";
    // For home page, exact match
    if (href === "/") {
      return pathname === href ? `${baseClass} text-primary` : baseClass;
    }
    // For other pages, check if pathname starts with href
    return pathname.startsWith(href) ? `${baseClass} text-primary` : baseClass;
  };

  return (
    <div className="py-8">
      <header className="max-w-[1110px] mx-auto px-6 md:px-0 flex justify-between items-center border-b border-white/20 text-white pb-8">
